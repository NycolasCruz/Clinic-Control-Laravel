...
<body>
    ...
    <div id="scripts">
        ...
        @if(session('update'))
        <script>
            swal('', '{{ session('update') }}', 'success')
        </script>
        @elseif(session('destroy'))
        <script>
            swal('', '{{ session('destroy') }}', 'success')
        </script>
        @endif
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                document.querySelector("form[id='form-register']").addEventListener("submit", async (event) => {
                    ...
                    if (validationFeedback() != false) {
                        try {
                            ...
                            const { data: response } = await axios.post("{{ route('criar') }}", new FormData(form));
                            const data = response.dados;

                            swal('', response['message'], 'success')
                            bootstrap.Modal.getOrCreateInstance(document.querySelector("#register")).hide();
                            form.reset();

                            const cardRegister = document.querySelector("#card-register");
                            const previousCards = cardRegister.innerHTML;
                            ...
                            if(error.responseJSON.errors) {
                                swal('Não Foi Possível Realizar Esta Ação!', error.responseJSON.errors.cpf[0], 'warning')
                            }
                        }
                    }
                });
